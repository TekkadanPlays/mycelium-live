...
export class BroadcastConfigTab extends Component<{}, BroadcastConfigTabState> {
  ...
  private handlePublishNow = async () => {
    ...
    try {
      // Build relay list from outbox + broadcast selections
      ...
      const { published, results } = await publishLiveEvent(event, relays);
      const accepted = Array.from(results.values()).filter((r) => r.accepted).length;
  ...
  render() {
    ...
    return (
      <div class="max-w-3xl space-y-6">
        <div>
          <h1 class="text-2xl font-bold text-foreground tracking-tight">Broadcast Configuration</h1>
          <p class="text-sm text-muted-foreground mt-1">
            Configure your NIP-53 live event metadata before going live. This information is published to Nostr relays so viewers can discover your stream.
          </p>
        </div>
        ...
        {!pubkey && (
          <Alert variant="destructive">
            <AlertDescription>
              Connect your Nostr identity in the <strong>Nostr Settings</strong> tab before configuring a broadcast.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Fill in the details that viewers will see when discovering your live stream on Nostr clients.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div class="space-y-4">
              ...
              {/* Summary */}
              <div>
                <label class="text-sm font-medium text-foreground mb-1.5 block">Summary</label>
                <textarea
              ...
              {/* Image URL */}
              <div>
                <label class="text-sm font-medium text-foreground mb-1.5 block">Cover Image URL</label>
                <Input
                  className="h-9"
              ...
              {/* Tags */}
              <div>
                <label class="text-sm font-medium text-foreground mb-1.5 block">Hashtags</label>
                  ...
                  <div class="flex flex-wrap gap-1.5 mt-2">
                    {config.tags.map((tag) => (
                      ...
                      >
                        <Badge
                          variant="secondary"
                          className="text-xs hover:bg-destructive/20 hover:text-destructive transition-colors pointer-events-none"
                        >
                          #{tag} ×
                        </Badge>
                      </span>
